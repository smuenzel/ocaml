...
module rec M : sig
  type t = int M.s
  type 'a s = 'a constraint 'a = M.t
end = M
[%%expect{|
Lines 1-4, characters 0-7:
1 | module rec M : sig
2 |   type t = int M.s
3 |   type 'a s = 'a constraint 'a = M.t
4 | end = M
Error: The definition of "M.s" contains a cycle:
         the 1st type parameter of "M.s" is constrained to "M.t",
         "M.t" = "int M.s"
|}];;
