...
let mem32 typ ?(scale = 1) ?base ?segment ?sym displ idx =
  assert(scale >= 0);
  Mem {arch = X86; typ; idx; scale; base; sym; displ; segment }

let mem64 typ ?(scale = 1) ?base ?segment ?sym displ idx =
  assert(scale >= 0);
  Mem {arch = X64; typ; idx; scale; base; sym; displ; segment }
